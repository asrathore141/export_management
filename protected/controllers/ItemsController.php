...
public function accessRules()
	...
	public function actionCreate()
	{
		...
		if(isset($_POST['Items']))
		{
			...
			$model->user_id = Yii::app()->user->getState('user')->user_id;
			 if ($model->save()){ 
			$photo = CUploadedFile::getInstance($model,'photo');
				if( $photo ){
					$save_location = YiiBase::getPathOfAlias("webroot").'/files/items/'.$model->item_id;
					if (!file_exists ($save_location)) {
						mkdir ($save_location, 0777, true);
					} 
					$photo->saveAs($save_location.'/'.$photo->getName());
					$model->photo = $photo;
					$model->update();
				}
                $this->redirect(array(
  ...
  public function actionPartialViews($id)
	{
		$this->renderPartial('_views1',array(
			'model'=>$this->loadModel($id),
		));
	}
